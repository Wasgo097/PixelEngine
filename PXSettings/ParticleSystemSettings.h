...
namespace Settings {
	struct ParticleSystemSettings {
		ParticleSystemSettings(const sf::Vector2f& position, const sf::Vector2f& gravity, const sf::Vector2f& size, float particle_speed, bool dissolve, unsigned char dissolution_rate)
			:origin{ position }, gravity{ gravity }, size{ size }, particle_speed{ particle_speed }, dissolve{ dissolve }, dissolution_rate{ dissolution_rate }{}
		ParticleSystemSettings() = default;

		bool operator==(const ParticleSystemSettings& obj)const {
			return origin == obj.origin and gravity == obj.gravity and size == obj.size and particle_speed == obj.particle_speed and dissolve == obj.dissolve and dissolution_rate == obj.dissolution_rate;
		}
