...
namespace Core{
	...
	public:
		...
		std::shared_ptr<type_to_create> SpawnActor(const Settings::ActorSettings & settings, sf::Vector2f velocity,Argv && ...argv){
			std::shared_ptr<type_to_create> result;
			type_to_create * ptr = new type_to_create(this, settings, velocity,std::forward<Argv>(argv)...);
			result.reset(ptr);
			ptr = nullptr;
