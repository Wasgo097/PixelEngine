...
namespace Core{
	Actor::Actor(World * world, const Settings::ActorSettings& actorsettings, const Settings::TextureSettings& texturesettings) :
		...
		if(!_texturesettings._texturepath.empty()){
			const std::string&  texturepath = _texturesettings._texturepath;
			if(!texturepath.empty()){
				...
				if(_texture->loadFromFile(texturepath)){
					...
					throw std::invalid_argument("Wrong path in actor constructor: " + texturepath + " in " + ToString()+" "+ _actorsettings.ToStdString());
				}
			}
		}
	}
