...
namespace Test {
	...
	void ControllerForForestTest::InitMainCharacterInputBindings() {
		...
		_actions[key] = [this](std::shared_ptr<Core::Object::ControlledActor> actor) {
			...
			//}
			//for (int i = 0; i < 4; i++)
			_world_parent->PushNewParticles(std::make_unique<Core::Particle::Smoke>(_world_parent->GetParticleEmitter(), *actor->GetPosition(), Factory::ParticleSystemSettings::Size::big));
		};
		key.repeatable = true;
