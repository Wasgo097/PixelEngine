...
std::unique_ptr<Core::Particle::Particle> Test::ExampleParticleSystem::CreateParticle() const {
	Settings::ParticleSettings settings;
	settings.color = sf::Color{ 255,255,255,255 };
	settings.max_time = 2.0f;
	settings.position.x = _randomizer.Random(.0, static_cast<double>(_settings.size.x));
	settings.position.y = _randomizer.Random(.0, static_cast<double>(_settings.size.y));
